...
export default {
  data () {
    return {
      chartInstance: null,
      allData: null,
      mapData: {} // 缓存所获取的省份的地图矢量数据，防止重复点击获取地图数据
    }
  },
  created () {
    // 在组件创建完成之后 进行回调函数的注册
  ...
  methods: {
    async initChart () {
      ...
      const initOption = {
        ...
        geo: {
          ...
          bottom: '5%',
          itemStyle: {
            areaColor: '#2E72BF', // 地图省份颜色
            borderColor: '#333'// 地图省份边界线颜色
          }
        },
        legend: {
          left: '5%', // 图例的位置
          bottom: '5%',
          orient: 'vertical' // 将图列竖向摆放
        }
      }
    ...
    getData (ret) {
      // 获取服务器的散点数据, 对this.allData进行赋值之后, 调用updateChart方法更新图表
